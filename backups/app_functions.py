...
def new_input(input_string=''):
    while True:
        ...
        if user_input in ["M", "m"]:
            ...
            if singletons.UserLoggedIn.user_type == 'manager':
                manager_program_start()
            elif singletons.UserLoggedIn.user_type == 'user':
                user_program_start()
        else:
...
def login_func(row_factory_cursor):
    ...
    singletons.UserLoggedIn.user_type = row_object['user_type']
...
def manager_submenu_input_director():
    ...
    for key in dict_of_lists_keys_list:
        if check_current_display(dict_of_lists[key]):
            ...
            break
    for key in dict_of_lists_keys_list:
        if check_current_menu(dict_of_lists[key]):
            current_mode = mode_key_dict[key]
            break
...
def manager_main():
        check_current_menu_image(["[1] View Table               [3] View Reports"])


def table_choice():
        check_current_menu_image(["[1] Users                    [2] Competencies"])
...
def director_prime():
    ...
    while True:
        ...
        # if check_current_menu(user_input) == "valid":
        #     break
        if check_current_menu(user_input) == "not valid":
            MOVE_TO_INPUT()
            print('\x1B[1K')
            print("\x1B[?25l")
            print("\x1B[A--Invalid Choice--")
    ...
    if user_input == '1' and manager_main() == True:
        manager_table_choice_menu_printer()
        while True:
            funcs_dict = {
                '1':manager_view_table_users,
    ...
    elif user_input == '2' and manager_main() == True:
        manager_user_profile_editor_screen()
