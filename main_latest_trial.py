...
def load_user_info(obj_dictionary=0, decrypted_password=0):
    if obj_dictionary not in [0,1, 2, 3, 4]:
        ...
        singletons.CurrentObject.object = row_object
    elif obj_dictionary == 2:
        chosen_id = new_input("Enter a competency_id: ")
        selected_users = CompetenciesTable(row_factory_cursor, 'competency_id')
        selected_user = selected_users.search_rows(chosen_id, 'exact')
        try:  
            row_keys = list(selected_user.keys())
        except:
        ...
        row_object = selected_user[row_keys[0]]
        # print(row_object)
        singletons.TablePage.table = "Competencies"
        singletons.CurrentCompetency.competency_id = str(row_object['competency_id'])
        singletons.CurrentCompetency.name = str(row_object['name'])
        singletons.CurrentCompetency.description = str(row_object['description'])
        singletons.CurrentCompetency.active = str(row_object['active'])
        singletons.CurrentObject.object = row_object
    elif obj_dictionary == 3:
        chosen_id = new_input("Enter a user_id: ")
        selected_users = UsersTable(row_factory_cursor, 'user_id')
        selected_user = selected_users.search_rows(chosen_id, 'exact')
        try:  
...
def input_director(manager_input_mode=0):
    ...
    while True:
        if manager_input_mode == 1:
            if load_user_info(obj_dictionary=2) != "user not found":
                if singletons.TablePage.table.lower() == 'users':
                    manager_user_profile_editor_printer()
                    manager_user_profile_editor_menu()
                elif singletons.TablePage.table.lower() == 'competencies':
                    manager_competency_editor_printer()
                    manager_competency_editor_menu()
                elif singletons.TablePage.table.lower() == 'assessments':
                    manager_assessment_editor_printer()
                    manager_competency_editor_menu()
                elif singletons.TablePage.table.lower() == 'assessment_results':
                    manager_assessment_results_editor_printer()
                    manager_assessment_results_editor_menu()
...
def table_users_f_name_update():
    if singletons.TablePage.table.lower() == 'users':
        ...
        update_in_editor('Name', 'Competencies')
        manager_competency_editor_printer()
        manager_competency_editor_menu()
    elif singletons.TablePage.table.lower() == 'assessments':
        pass
...
def table_users_l_name_update():
    if singletons.TablePage.table.lower() == 'users':
        ...
        update_in_editor('Description', 'Competencies')
        manager_competency_editor_printer()
        manager_competency_editor_menu()
...
def manager_view_table_assessments():
    ...
    singletons.TablePage.table == 'Assessments'
    manager_view_table()
